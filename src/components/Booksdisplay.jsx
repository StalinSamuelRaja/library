...
import { Deletebooks } from "../api/Booksapi.js";
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 m-5 p-5">
        {info&& info?.map((data,idx)=>(
           ...
           <div className="card-body">
             <h2 className="card-title"> <kbd className="kbd bg-accent-content">Title:{data.title}</kbd></h2>
             <h3 className="card-title"><kbd className="kbd bg-neutral">Author:{data.author}</kbd> </h3>
             <h2 ><kbd className="kbd bg-accent-content">Date Published:{data.publicationdate}</kbd> </h2>
             <h2><kbd className="kbd bg-neutral">ISBN No:{data.isbn}</kbd> </h2>
             <div className="card-actions justify-end">
               <button className="btn btn-success" onClick={()=>navigate(`/booksedit/${data.id}`)}>Edit </button>
